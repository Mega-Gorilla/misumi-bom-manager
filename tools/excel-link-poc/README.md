...
case-05a/05b/07 (参考)       pause 窓競合4シナリオで競合コピーの生成は観測されず。勝者=後から再開した端点。
                             敗者は版履歴のみ（Lost & Found 含め別名ファイルはゼロ）
                             ※ case-05b はアプリ側端点が無変化のまま remote-only 版が履歴へ落ちた =
                               指紋照合でも検出できない反例（残余リスクとして plan.md §3.12 に固定）
case-91/92 プレゼンスON      case-91/92 では OFF 時との差を観測せず（警告・表示ゼロ）
```

**鍵となる知見**（今回の構成 — Drive v128 ミラー・同一アカウント2PC — での観測）: Drive は競合コピーを
作らず**後勝ち線形化＋版履歴**で解決した。Drive/Office からの警告は一切なく、**合否対象の範囲
（単一ユーザーの順次操作＋同期遅延）ではアプリの指紋照合が事実上唯一の検出手段**（§4.6.2/§4.2.2 の設計が
必然）。ただし case-05b のとおり**非対応シナリオ（同時変更）には指紋でも検出不能な消失が残る** —
GO の適用範囲は合否対象に限られ、「同時編集非対応」の運用警告が必須。Google Sheets で**編集・復元操作を
した後**にバイト列が再シリアライズされ指紋が変わることを実測（int→float。閲覧のみは未検証）—
fail-safe 方向の誤検出として仕様に明記する。テストルートは sync-guard 検証つきクリーンアップで削除済み・
残骸なし。**各ケースの `log.jsonl` はテストルートと共に削除された**ため、監査可能な証跡は
`docs/plans/0018-excel-link-mode/3c-results.md`（機微除去済みの実測記録・Issue #23 コメントへのリンク付き）に
コミットしてある。ローカル原本は `out/syncpoc-results.md`（gitignore）。
